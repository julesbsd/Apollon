...
- `exercises_library/{exerciseId}` - lecture publique (`if true`), ecriture `if isAuthenticated()` (import via script Admin SDK). Documents : Exercice du catalogue, provenant de deux sources. Les exercices de l'API WorkoutAPI ont leur ID d'origine (UUID). Les exercices custom (machines Fitness Park supplementaires, autres equipements) ont un ID prefixe `custom_` (ex: `custom_low_row`, `custom_seated_dip`). Tous les exercices possedent un champ `hasImage: bool` qui indique si une image est disponible (asset top-20, local telecharge, ou remote API). Les exercices custom ont generalement `hasImage: false`.
...
Scripts d'import (dossier `scripts/`, Node.js + firebase-admin) : `npm run seed` et `npm run import-library` pointent tous deux vers `import_exercise_library.js`, qui importe le catalogue d'exercices dans `exercises_library` a partir de deux sources JSON. Source 1 : `docs/workout_api_exercises_fr.json` (obligatoire, exercices WorkoutAPI ~94). Source 2 : `docs/custom_exercises_fr.json` (optionnel, exercices supplementaires machines Fitness Park absentes, etc., IDs prefixes `custom_` pour distinguer/eviter collisions ; POURQUOI des IDs stables : l'API WorkoutAPI regenere periodiquement ses IDs — rotation observee, voir scripts/README-SEED.md — donc un exercice custom ne s'appuie sur aucun ID API). Si source 2 absente, import poursuit (message informatif). Collisions d'IDs detectees en phase locale (avant Firestore) : arret script et pas d'ecriture incomplete. Les scripts divergents (seed_exercises_node.js, seed_exercises.dart, seed_exercises_standalone.dart), qui ecrivaient a tort dans la collection `exercises`, ont ete archives dans `scripts/_archive/`. L'auth se fait via `serviceAccountKey.json` (ne JAMAIS committer cette cle).
